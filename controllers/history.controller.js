...
const getVideosInHistory = async (req, res) => {
  try {
...
const saveVideoToHistory = async (req, res) => {
  try {
    ...
    res.json({
      success: false,
      message: "Unable to add to watch history",
      errorMessage: err.message,
    });
  }
};
...
const removeVideoFromHistory = async (req, res) => {
  try {
    ...
    const deletedVideoId = await history.findOneAndDelete({
